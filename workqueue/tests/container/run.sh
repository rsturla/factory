...
#
# Expects pre-built container images (from Konflux or local build).
...
#
# When triggered by Konflux Testing Farm task, these env vars are automatically
# set to the images built in that pipeline run.
#
set -euo pipefail
