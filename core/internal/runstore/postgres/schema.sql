-- Factory core schema for pipeline runs, stages, audit, and outbox.
-- Lives in same PostgreSQL instance as workqueue, separate schema.
...
CREATE INDEX IF NOT EXISTS idx_stage_runs_phase ON factory.stage_runs(phase);
...
CREATE INDEX IF NOT EXISTS idx_outbox_sent ON factory.outbox(sent, created_at) WHERE sent = false;
