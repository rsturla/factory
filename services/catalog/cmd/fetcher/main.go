...
import (
	...
	_ "modernc.org/sqlite" // Syft requires a SQLite driver to read RPM databases (RHEL 9+ uses SQLite for rpmdb)
...
func main() {
	...
	analyzeReceiverURL := envOr("ANALYZE_RECEIVER_URL", "http://localhost:8081")
	analyzeQueue := envOr("ANALYZE_QUEUE", "catalog-analyze")
	...
	scanner := fetch.NewSyftScanner()
	enqueueClient := reconciler.NewEnqueueClient(analyzeReceiverURL)
	rec := fetch.NewReconciler(s, blobs, scanner, enqueueClient, analyzeQueue)
